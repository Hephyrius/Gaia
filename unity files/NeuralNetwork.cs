...
public class NeuralNetwork : IComparable<NeuralNetwork>
{
    ...
    public float[] FeedForward(float[] inputs)
    {
        ...
        for (int i = 1; i < layers.Length; i++)
        {
            for (int j = 0; j < neurons[i].Length; j++)
            {
                ...
                //neurons[i][j] = (float) Sigmoid((float) value);
            }
        }
    ...
    //CreateCrossover weights from 2 parent networks
    public void CrossoverWeights(float crossoverRate, NeuralNetwork One, NeuralNetwork Two)
    {
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    if (j < (weights[i].Length/2))
                    {
                        weights[i][j][k] = One.weights[i][j][k];
                    }
                    else
                    {
                        weights[i][j][k] = Two.weights[i][j][k];
                    }
                }
            }
        }
    ...
    //CreateCrossover weights from 2 parent networks
    public void CrossoverTwoWeights(float crossoverRate, NeuralNetwork One, NeuralNetwork Two)
    {
        bool toggle = false;
        for (int i = 0; i < weights.Length; i++)
        {
    ...
    //CreateCrossover weights from 2 parent networks
    public void CrossoverThreeWeights(float crossoverRate, NeuralNetwork One, NeuralNetwork Two)
    {
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    weights[i][j][k] = (Two.weights[i][j][k] + One.weights[i][j][k] )/2;
    ...
    //CreateCrossover weights from 2 parent networks
    public void CrossoverFourWeights(float crossoverRate, NeuralNetwork One, NeuralNetwork Two)
    {
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    weights[i][j][k] = (float) Math.Sqrt((double)(Two.weights[i][j][k] * One.weights[i][j][k]));
